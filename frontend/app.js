...
function noteApp() {
    return {
        ...
        async onEditorDrop(event) {
            ...
            if (event.dataTransfer && event.dataTransfer.files && event.dataTransfer.files.length > 0) {
                ...
                if (mdFiles.length) {
                    ...
                    const openNote = this.currentNote || '';
                    const noteFolder = openNote.includes('/') ? openNote.slice(0, openNote.lastIndexOf('/')) : '';
                    await this.handleFileUploadDrop(dt.files, true, noteFolder);
                }
        ...
        async handleFileUploadDrop(files, fromDragDrop = false, targetFolderOverride = null) {
            ...
            try {
                ...
                // Editor drops pass the open note's folder.
                let targetFolder;
                if (targetFolderOverride !== null) {
                    targetFolder = targetFolderOverride;
                } else if (!fromDragDrop && this.dropdownTargetFolder !== null && this.dropdownTargetFolder !== undefined) {
        ...
        onUploadDragEnter(e) {
            ...
            // The textarea editor handles its own file drops (media inline, .md via the
            // upload dialog), so the full-screen overlay must not cover it.
            if (e.target?.closest?.('.editor-textarea')) return;
...
`;
        ...
        setupScrollSync() {
            ...
            // Smart (anchor) sync measures the classic textarea, so Live Preview always
            // uses percentage sync through the editor adapter.
            const editor = classicEditor;
            const useAnchors = () => this.smartScrollSync && !liveEditor && !!editor;
        ...
        _restoreNoteScroll() {
            ...
            if (preview) {
                ...
                if (scrollable > 0) {
                    if (this.smartScrollSync && this.editorMode !== 'live-preview' && editor) {
                        const anchors = this._getScrollAnchors();
                        const totalLines = (this.noteContent || '').split('\n').length;
                        this.isScrolling = true;
